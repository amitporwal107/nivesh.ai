...
export function useSendChat() {
    ...
    return useMutation({
        mutationFn: (args) => chatService.send(args.message, args.sessionId),
        onSuccess: (_data, vars) => {
            qc.invalidateQueries({ queryKey: ["chat", "sessions"] });
            if (vars.sessionId)
                qc.invalidateQueries({ queryKey: ["chat", "sessions", vars.sessionId] });
        },
    });
}
export function useCreateChatSession() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (title) => chatService.createSession(title),

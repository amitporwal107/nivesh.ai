/**
 * Mock dashboards adapter — synthesises the v4 envelope from existing mocks.
 */
import type { DashboardsAdapter, DashboardDomain } from "../adapters/dashboards.adapter";
...
export const mockDashboardsAdapter: DashboardsAdapter = {
  async fetch(domain: DashboardDomain) {
    await delay(280);
    ...
    const envelopes: Record<DashboardDomain, unknown> = {
      concentration: {
        type: "concentration",
        badge: "ATTENTION",
    ...
    return structuredClone(envelopes[domain] as DashboardEnvelopeImport);
  },
};

// helper to keep TS happy — the structuredClone return is typed via assertion
type DashboardEnvelopeImport = import("../contracts/dashboard.contract").DashboardEnvelopeC;

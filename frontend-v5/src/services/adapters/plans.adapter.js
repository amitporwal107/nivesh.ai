...
import { PlanC, PlanActiveRes, PlanGenerateRes, PlanActionUpdateRes, PlanHealthProjectionRes, SignalsRes, } from "@/services/contracts/plan.contract";
export const realPlansAdapter = {
    async getActive() {
        const res = await http({ path: "/api/plans/active" });
        const parsed = PlanActiveRes.safeParse(res.data);
        if (!parsed.success)
            throw ApiError.contractDrift(`plans.active: ${parsed.error.message}`);
    ...
    async getPlan(planId) {
        ...
        if (!parsed.success)
            throw ApiError.contractDrift(`plans.getPlan: ${parsed.error.message}`);
        return parsed.data;
    },
    async generate() {
        ...
        if (!parsed.success)
            throw ApiError.contractDrift(`plans.generate: ${parsed.error.message}`);
        return parsed.data.plan;
    },
    async refresh() {
    ...
    async healthProjection() {
        ...
        if (!parsed.success)
            throw ApiError.contractDrift(`plans.healthProjection: ${parsed.error.message}`);
        return parsed.data;
    },
    ...
    async updateActionStatus(planId, actionId, status, completion_note) {
        ...
        if (!parsed.success)
            throw ApiError.contractDrift(`plans.updateActionStatus: ${parsed.error.message}`);
        return parsed.data;
    },
    ...
    async generateSignals() {
        ...
        if (!parsed.success)
            throw ApiError.contractDrift(`signals.generate: ${parsed.error.message}`);
        return parsed.data;
    },
 ...
 *   hold                          → KEEP
 */
function mapActionToRecommendation(a) {
    const recAction = a.action_type === "sell" || a.action_type === "switch" || a.action_type === "sip_decrease" ? "reduce" :

...
export interface DashboardsAdapter {
  fetch(domain: DashboardDomain, params?: Record<string, string | number>): Promise<DashboardEnvelopeC>;
}

export const realDashboardsAdapter: DashboardsAdapter = {
  async fetch(domain, params) {
    ...
    if (!parsed.success) throw ApiError.contractDrift(`dashboards.${domain}: ${parsed.error.message}`);

...
import {
  ...
  WhatIfRes,
  FundShortlistRes,
} from "@/services/contracts/goal.contract";
...
export interface GoalsAdapter {
  list(): Promise<{ goals: DomainGoal[]; totals: { total: number; onTrack: number; atRisk: number } }>;
  get(goalId: string): Promise<import("@/services/contracts/goal.contract").GoalC>;
  create(input: import("@/services/contracts/goal.contract").GoalC): Promise<DomainGoal>;
  update(goalId: string, body: Partial<import("@/services/contracts/goal.contract").GoalC>): Promise<unknown>;
  archive(goalId: string): Promise<{ ok: true }>;
  getSnapshot(): Promise<import("@/services/contracts/goal.contract").FinancialSnapshotC | null>;
  upsertSnapshot(body: Partial<import("@/services/contracts/goal.contract").FinancialSnapshotC>): Promise<import("@/services/contracts/goal.contract").FinancialSnapshotC>;
  simulate(goalId: string): Promise<import("@/services/contracts/goal.contract").MonteCarloRes>;
  whatIf(goalId: string, body: import("@/services/contracts/goal.contract").WhatIfReq): Promise<import("@/services/contracts/goal.contract").WhatIfRes>;
  fundShortlist(bucket: string, n?: number, minQuality?: number): Promise<import("@/services/contracts/goal.contract").FundShortlistRes>;
  copilotAsk(goalId: string, query: string): Promise<{ answer: string; engineOutput?: unknown }>;
  copilotHistory(goalId: string): Promise<Array<{ query: string; answer: string; asked_at: string }>>;
}

export const realGoalsAdapter: GoalsAdapter = {
  async list() {
  ...
  async get(goalId) {
    ...
    if (!parsed.success) throw ApiError.contractDrift(`goals.get: ${parsed.error.message}`);
    return parsed.data;
  },

  async create(input) {
    const body = GoalCreateReq.parse({
      goal_type:         input.goal_type as "retirement"|"education"|"home"|"emergency"|"wealth"|"other",
    ...
    const parsed = GoalC.safeParse(res.data);
    if (!parsed.success) throw ApiError.contractDrift(`goals.create: ${parsed.error.message}`);
  ...
  async upsertSnapshot(body) {
    ...
    const parsed = FinancialSnapshotC.safeParse(res.data);
    if (!parsed.success) throw ApiError.contractDrift(`goals.upsertSnapshot: ${parsed.error.message}`);
    return parsed.data;
  },
  ...
  async simulate(goalId) {
    ...
    if (!parsed.success) throw ApiError.contractDrift(`goals.simulate: ${parsed.error.message}`);
    return parsed.data;
  },
  ...
  async whatIf(goalId, body) {
    ...
    if (!parsed.success) throw ApiError.contractDrift(`goals.whatIf: ${parsed.error.message}`);
    return parsed.data;
  },
  ...
  async fundShortlist(bucket, n = 15, min_quality = 55) {
    ...
    if (!parsed.success) throw ApiError.contractDrift(`goals.fundShortlist: ${parsed.error.message}`);
    return parsed.data;
  },
  ...
  async copilotAsk(goalId, query) {
    ...
    const obj = res.data as { answer?: string; engine_output?: unknown };
  ...
  async copilotHistory(goalId) {
    ...
    const obj = res.data as { history?: Array<{ query: string; answer: string; asked_at: string }> };
    return obj.history ?? [];
  },
};

...
import { HoldingsListRes, TrendRes, EnrichedHoldingsRes, InstrumentSearchRes, SipsListRes, } from "@/services/contracts/portfolio.contract";
...
export const realPortfolioAdapter = {
    async listHoldings(portfolioId, assetType) {
        ...
        if (!parsed.success)
            throw ApiError.contractDrift(`portfolio.listHoldings: ${parsed.error.message}`);
    ...
    async listHoldingsEnriched(fresh = false) {
        ...
        if (!parsed.success)
            throw ApiError.contractDrift(`portfolio.enriched: ${parsed.error.message}`);
        return parsed.data;
    },
    async getSummary() {
    ...
    async getNavHistory(range) {
        ...
        if (!parsed.success)
            throw ApiError.contractDrift(`portfolio.getNavHistory: ${parsed.error.message}`);
    ...
    async searchInstruments(q) {
        ...
        if (!parsed.success)
            throw ApiError.contractDrift(`portfolio.searchInstruments: ${parsed.error.message}`);
        return parsed.data;
    },
    async listSips() {
        ...
        if (!parsed.success)
            throw ApiError.contractDrift(`portfolio.listSips: ${parsed.error.message}`);
        return parsed.data;
    },
};

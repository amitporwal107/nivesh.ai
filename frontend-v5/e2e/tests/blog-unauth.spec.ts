...
test.describe("Blog", () => {
  ...
  test("TC-11: /blog index lists the flagship 'team of ten' overview", async ({ page }) => {
    await page.goto("/v5/blog");
  ...
  test("TC-12: homepage 'From the blog' section shows the flagship card", async ({ page }) => {
    await page.goto("/v5/");
  ...
  test("TC-13: flagship article renders the rule, gate terminal, six blocks and honest scorecard", async ({ page }) => {
  ...
  test("TC-14: flagship CTA 'Try Nivesh Copilot free' navigates to /login", async ({ page }) => {
    await page.goto(`/v5/blog/${T10_SLUG}`);
    await page.getByTestId("blog-article").waitFor({ state: "attached", timeout: 90000 });
  ...
  test("TC-15: flagship cross-links open the two engineering deep-dives", async ({ page }) => {
    await page.goto(`/v5/blog/${T10_SLUG}`);
    await page.getByTestId("blog-article").waitFor({ state: "attached", timeout: 90000 });

    ...
    <ScreenContainer variant={isDesktop ? "desktop" : "mobile"}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: isDesktop ? 26 : 18, paddingBottom: 40 }}>
        ...
        <HeroCard
          ...
          description={isDesktop ? `Invested ${inrCompact(p.summary.investedValue)} · ${pct(p.summary.delta1dPct, 2)} today · XIRR ${p.summary.xirr}%.` : null}
          viz={
            <HeroVizPanel eyebrow="Allocation" value={`${p.allocation[0].value}%`} unit="equity" size={isDesktop ? "desktop" : "mobile"}>
        ...
        <section>
          <SectionHead title="Holdings" count={`${p.topHoldings.length} funds`} />

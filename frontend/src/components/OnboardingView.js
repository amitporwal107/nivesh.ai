...
// Existing-investor flow is just two steps now: pick the journey, then
// open the casparser.in Portfolio Connect SDK (which handles broker /
// PDF / Gmail in one widget) and on success land on the playbook.
...
const OnboardingView = ({ onComplete, userProfile }) => {
  ...
  // Gmail picker state + ?gmail=connected handler were removed when we
  // consolidated CAS / Gmail / broker imports into the Connect SDK
  // widget (it runs its own Gmail OAuth in-browser and returns parsed
  // JSON straight to /api/portfolio/import-connect).
  ...
  // pollUploadStatus + handleFileUpload were the legacy direct-PDF
  // upload helpers. Replaced by the Connect SDK widget (CasUploadButton)
  // which posts parsed JSON straight to /api/portfolio/import-connect.

  // ── Removed Gmail handlers ────────────────────────────────────────
  // handleGmailConnect / scanGmailEmails / importSelectedGmailEmails /
  // pollSingleTask / _casRecencyScore / toggleGmailSelection were all
  // server-side Gmail picker plumbing. Replaced by the Connect SDK
  // widget's enableInbox flow (CasUploadButton handles everything).
    ...
    <div className="max-w-2xl w-full mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          One button opens our secure import widget — connect your broker (CDSL), upload a CAS PDF, or pull statements from Gmail. Pick whichever is fastest for you.
        </p>
      </div>

      {/* Single Connect SDK card — the widget surfaces all three import
          modes (Connect Broker via CDSL OTP · Upload File · Gmail Inbox)
          in one modal, so we no longer fork the UI per data source. */}
      <Card
        data-testid="source-connect-sdk"
        className="cursor-default p-6 rounded-xl border-2 border-emerald-200 dark:border-emerald-800 bg-gradient-to-br from-emerald-50/80 via-blue-50/40 to-violet-50/30 dark:from-emerald-900/20 dark:via-blue-900/10 dark:to-violet-900/10 hover:border-emerald-400 transition-all mb-6"
      >
        <div className="grid grid-cols-3 gap-3 mb-5">
          <div className="text-center">
            ...
            <div className="text-xs font-semibold text-slate-800 dark:text-slate-200">Connect Broker</div>
            <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">CDSL OTP — live holdings</div>
          </div>
          <div className="text-center">
            <div className="w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center mx-auto mb-2">
              <FileText className="w-5 h-5 text-blue-600" strokeWidth={1.5} />
            </div>
            <div className="text-xs font-semibold text-slate-800 dark:text-slate-200">Upload CAS PDF</div>
            <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">NSDL · CDSL · CAMS · KFin</div>
          </div>
          <div className="text-center">
            <div className="w-10 h-10 rounded-xl bg-violet-100 dark:bg-violet-900/40 flex items-center justify-center mx-auto mb-2">
              <Mail className="w-5 h-5 text-violet-600" strokeWidth={1.5} />
            </div>
            <div className="text-xs font-semibold text-slate-800 dark:text-slate-200">Connect Gmail</div>
            <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Auto-fetch CAS emails</div>
          </div>
        </div>
        ...
        <p className="text-[10px] text-slate-400 dark:text-slate-500 text-center mt-3">
          Read-only access — your credentials never leave the casparser.in widget.
        </p>
      </Card>
  ...
  // renderUpload (legacy direct PDF upload) was removed when the
  // onboarding data-source step was unified into the Connect SDK
  // widget. CasUploadButton in renderDataSource handles all imports.


  // renderGmailPicker + renderGmailTimeline were removed when the
  // data-source step was unified into the Connect SDK widget. The
  // widget handles Gmail import in-browser via enableInbox.

...
## TC-10 — backlog drain against real `nidp_staging` (IN PROGRESS)

The committed branch was cloned onto nidp-stack-vm via git (`/tmp/cls-drain`, commit
`104338a`) and run against the real DB with `/opt/nidp/nidp.env` sourced for credentials and
`GROQ_API_KEY` supplied as an ephemeral env var (never written to disk).

Dry run first (5 rows, zero writes) — real output:

```
announcement_classifier provider=groq model=openai/gpt-oss-120b version=openaigptoss120b-0216a46c04
HTTP Request: POST https://api.groq.com/openai/v1/chat/completions "HTTP/1.1 200 OK"
[dry-run] NSE_ANN/0df93e7933 → other/low/neutral        Copy of Newspaper Publication
[dry-run] NSE_ANN/6572ba7e9f → other/low/neutral        Updates
[dry-run] NSE_ANN/bccba8d09b → other/low/neutral        Analysts/Institutional Investor Meet/Con. Call Updates
[dry-run] NSE_ANN/a261b49a49 → management/medium/neutral  Press Release
[dry-run] NSE_ANN/faa020bcb4 → management/medium/neutral  Appointment
classifier done: processed=5 errors=0 duration=4.0s categories={'other': 3, 'management': 2} impact={'low': 3, 'medium': 2}
```

Then launched detached (`setsid`, so it survives my GCP token expiring). Real writes landing:

```
unclassified | classified      (30-day window)
        4808 |       3801      ← before drain
        4764 |       3861
        4746 |       3879
```
...
```
classified: 3898 -> 3911  in 60s  => 13 rows/min
```

**~13 rows/min ⇒ ~6 h for the remaining ~4,746 rows.** That is far below the 0.9 s/row
measured locally, and no `classify failed` errors are logged — the limiter is almost
certainly Groq's free-tier **tokens**-per-minute (the system prompt is ~1.6K tokens on every
call), which is account-wide, so sharding would not help. `fetch_unclassified` has no shard
parameter anyway (unlike `embed_pending`), so parallel workers would re-fetch the same rows.

The job is unattended and self-terminating (stops when the queue empties). The page improves
continuously as rows land.

## Data Correctness (staging)

**PARTIAL — drain still running at end of turn.** Verified so far: rows are being written
with real `event_category`/`impact_score`/`sentiment` by the Groq path (counts above moving
in the right direction, 0 errors). NOT yet verified: full backlog cleared, and the
`/v5/research` UI (TC-11, needs a session token). **No claim is made that the feed is fixed.**

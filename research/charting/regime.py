...
def _adx_series(bars: pd.DataFrame, period: int = 14) -> pd.Series:
    """Wilder's ADX. The implementation moved to `series.adx` on 2026-09-23 when the chart indicator
    catalogue began serving it; this stays as the regime-local name (and its `adx_<period>` series
    name, which `trend_context` and `trend_classification` key off) so there is exactly one
    implementation and no call site had to change."""
    return series.adx(bars, period).rename(f"adx_{period}")

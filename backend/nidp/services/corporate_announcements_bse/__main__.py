"""python -m nidp.services.corporate_announcements_bse [--date YYYY-MM-DD]

Thin wrapper around the shared `corporate_announcements` package that
pins source='bse' and registers run-status to nidp.job_log via JobRun
...
def main() -> None:
    ...
    a = p.parse_args()
    setup_logging(service="corporate_announcements_bse")

"""Run the Parquet exporter with JobRun tracking."""
...
def main() -> None:
    p = argparse.ArgumentParser(description="Export WARM-tier tables to MinIO as Parquet")
    p.add_argument("--date", type=_parse_date, default=None,
                   help="Export date (YYYY-MM-DD). Default = yesterday.")
    args = p.parse_args()
    setup_logging(service="parquet_exporter")

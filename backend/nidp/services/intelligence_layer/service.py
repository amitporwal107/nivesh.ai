...
async def run(target_date: date | None = None) -> dict[str, int | str]:
    ...
    async with pool.acquire() as conn:
        ...
        eq_rows = await conn.execute(
            ...
            SELECT DISTINCT
                'EQUITY', 'EQUITY', p.symbol, p.isin, p.symbol,
                COALESCE(ic.company_name, p.symbol),
                NULL::text,
                ic.industry,
                'nidp.prices_eod'
        ...
        # 2) Security master: mutual funds from mf_scheme_master.
        mf_rows = await conn.execute(
            ...
            SELECT DISTINCT
                'MF_SCHEME', 'MUTUAL_FUND', s.scheme_code,
                COALESCE(s.isin_growth, s.isin_idcw), s.scheme_code,
                s.scheme_name,
        ...
        corr_rows = await conn.execute(
            ...
            daily_ret AS (
                ...
                WHERE p.source = 'NSE_BHAVCOPY'
                  AND p.as_of_date >= COALESCE($1::date - INTERVAL '120 days', (SELECT dt - INTERVAL '120 days' FROM latest))
            ),
            pairs AS (
                SELECT

...
CREATE UNIQUE INDEX IF NOT EXISTS ux_user_holdings_snapshot_dedup
    ON portfolio.user_holdings_snapshot (
        external_user_id, snapshot_date,
        COALESCE(isin, ''), COALESCE(symbol, ''), COALESCE(amfi_scheme_code, ''),
        source_system
    );

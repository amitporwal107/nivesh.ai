-- 101_mf_scorecard_fund_level_aum.sql
--
-- Builds on migration 100. Now that scheme-wise AAUM is populated per plan, the
-- scorecard's aum_cr is the FUND-level total (sum of all plan/option AAUMs for a
-- fund) instead of a single plan's AUM — so the card shows e.g. HDFC Balanced
-- Advantage ~Rs 1.06L Cr, not just the Regular-Growth plan's ~Rs 54k Cr.
--
...
ident AS (
    SELECT c.*,
        ...
        -- Fund-level AUM = sum of every plan/option's scheme AAUM (migration 101).
        (SUM(c.aum_cr) OVER (
            PARTITION BY c.sub_category, c.amc_code,
                lower(regexp_replace(
                    c.scheme_name,
                    '\s*[-–]?\s*\y(direct|regular|growth|idcw|dividend|payout|reinvest\w*|bonus|plan|option|daily|weekly|monthly|quarterly|half.?yearly|annual|fortnightly)\y.*$',
                    '', 'i'))
        ))::numeric(14,2) AS fund_aum_cr
    FROM combined c
),
...
SELECT
    ...
    i.ter, i.fund_aum_cr AS aum_cr, i.top10_concentration_pct,
...
VALUES ('101_mf_scorecard_fund_level_aum.sql')

-- 062_fix_compute_mf_derived_analytics.sql
--
-- Repo-commits the hot-patch that was applied in prod to fix migration 051's
-- compute_mf_derived_analytics() function.
--
-- Why this exists:
--   The original AUM-trend OLS block in 051 was:
--       SELECT COUNT(*), AVG(x), AVG(y),
--              SUM((x - AVG(x) OVER ()) * (y - AVG(y) OVER ())),
--              SUM((x - AVG(x) OVER ()) ^ 2)
--         INTO v_aum_obs, v_mx, v_my, v_num, v_den FROM aum_pts;
--   The SUM(...) expressions contain AVG(...) OVER () inside them — a
--   window function nested under an aggregate. Postgres rejects this with:
--       ERROR: aggregate function calls cannot contain window function calls
--   compute_mf_derived_analytics() aborts the moment a scheme with >= 3
--   AUM points is processed, taking the whole mf_derived_refresh job with it.
--
-- The fix:
--   Postgres has built-in OLS aggregates that compute these sums directly:
--       REGR_SXY(y, x) = SUM((x - mean(x)) * (y - mean(y)))
--       REGR_SXX(y, x) = SUM((x - mean(x))^2)
--   These are pure aggregates — no nested window calls, no error.
--   Same mathematical result as the original intent.
--
-- History:
--   - 2026-05-20 — applied directly to prod via CREATE OR REPLACE FUNCTION
--     (undocumented hot-patch); mf_derived_refresh started returning OK.
--   - 2026-05-21 — committed to repo as migration 062 so a fresh DB rebuild
--     keeps the fix (otherwise rerunning 051 would re-introduce the bug).
--
-- Idempotent: CREATE OR REPLACE FUNCTION is a no-op if the body already
-- matches. Safe to run after 051 has already been applied.

SET search_path TO nidp, public;

CREATE OR REPLACE FUNCTION nidp.compute_mf_derived_analytics(p_scheme_code text, p_as_of_date date, p_window_days integer DEFAULT 1095)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- FIX (migration 062): replaced SUM((x - AVG(x) OVER ()) * ...) with
    -- REGR_SXY / REGR_SXX which compute the same OLS quantities as pure
    -- aggregates (no aggregate-containing-window error).
    WITH aum_pts AS (
        SELECT
...
VALUES ('062_fix_compute_mf_derived_analytics.sql')

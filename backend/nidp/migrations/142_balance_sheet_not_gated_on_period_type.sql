-- 142_balance_sheet_not_gated_on_period_type.sql
-- current_ratio stayed NULL for 707 symbols whose balance sheet is fully populated.
-- v_stock_fundamentals_latest's bs_annual CTE filtered period_type = 'annual', but the
-- fiscal-year-end balance sheet very often lives on the March-quarter row, typed
-- 'quarterly' (RELIANCE: current assets 594,249 cr on a 'quarterly' 2026-03-31 row).
-- The view never saw those values. The sibling cf_annual CTE carries no such filter,
-- which is exactly why cfo_pat_ratio reached 965 symbols while current_ratio stalled at 581.
--
-- Balance-sheet items are point-in-time (instant) values that sit on the year-end row
-- however that row is typed, so the filter is dropped rather than retyping the rows --
-- retyping the March quarter to 'annual' would pull Q4 out of every TTM sum.
--
-- Generated from the live definition, so migration 140's four-quarter TTM guard is kept.

...
--   WHERE scheme_code = p_scheme_code ORDER BY as_of_date  (range scan)
-- A single (scheme_code, as_of_date DESC) index covers both patterns.
--
-- Also adds a composite index on (as_of_date DESC) alone for the
-- bulk-batch outer loop that ORDER BY as_of_date DESC across all schemes.
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_mf_nav_daily_scheme_date
    ON nidp.mf_nav_daily (scheme_code, as_of_date DESC);

-- Index 2: latest-NAV look-ups (WHERE as_of_date = $latest, all schemes)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_mf_nav_daily_date_scheme
    ON nidp.mf_nav_daily (as_of_date DESC, scheme_code);

-- 136_fix_pb_book_value_per_share.sql
--
-- pb was 0.00 for every stock that had one. populate_stock_features_extended
-- divided close by (total_equity_cr * 1e7 / face_value / 1e7 * 1e7): net worth
-- used as if it were a share count, so book value per share came out ~10^7 too
-- large and pb rounded to zero. Shares = equity_share_capital / face_value (the
-- basis shares_outstanding and market_cap_cr already use in this function), so
--     book value per share = total_equity_cr * face_value / equity_share_capital_cr   (Rs/share)
-- The body is the live staging definition (pg_get_functiondef, 2026-09-11); only
-- the pb expression changes. Re-runnable.
...
VALUES ('136_fix_pb_book_value_per_share.sql')

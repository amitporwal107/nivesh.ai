-- 106_fix_psfv3_quarterly_period_type_case.sql
-- SD-08 follow-up: populate_stock_features_v3's quarterly CTE filtered
--   period_type = 'QUARTERLY'  (uppercase only)
-- but Screener writes lowercase 'quarterly' too — 1,096 symbols have ONLY lowercase
-- quarterly rows and were silently excluded from profit_margin_trend / debt_trend.
-- The annual CTE already handled both cases; this makes the quarterly CTE match.
...
BEGIN
    ...
    q_ranked AS (
        SELECT *,
               ...
               MAX(consolidated::INT) OVER (PARTITION BY symbol) AS has_any_consol
          FROM nidp.nse_financials_quarterly
         WHERE UPPER(period_type) = 'QUARTERLY'
    ),
    q_pref AS (
...
VALUES ('106_fix_psfv3_quarterly_period_type_case.sql')

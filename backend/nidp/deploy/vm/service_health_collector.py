...
SERVICES: list[ServiceDef] = [
    ...
    ServiceDef("nidp-stack-vm", "timescaledb-prod", "http://127.0.0.1:5433",    environment="prod"),   # port check
    ServiceDef("nidp-stack-vm", "timescaledb-stg",  "http://127.0.0.1:5434",    environment="staging"),

    # ── nivesh-app-vm: prod stack (probed externally via HTTPS) ───────────
...
def _probe(svc: ServiceDef) -> dict:
    ...
    # Special-case raw TCP port probes (postgres)
    if parsed.path == "" or (parsed.path == "" and parsed.scheme in ("", None)):
        import socket

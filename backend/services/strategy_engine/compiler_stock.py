...
#
# Environment-aware: when full NIDP `index_constituents` is deployed
# we'll use point-in-time lookups. For now this env's curated universe
# lives in `stock_master` (226 large/midcaps with is_nifty_100=TRUE).
# That's our pragmatic Phase-1 stand-in for the entire Nifty family —
# the same 226 symbols are what the positional engine and V3 already
# operate on. NIFTY50 narrows to top-50 by market cap.
INDEX_REFS = {
    ...
    "NIFTY200": "sm.is_nifty_100 IS TRUE",   # placeholder: this env covers ~226
    "NIFTY500": "sm.is_nifty_100 IS TRUE",   # placeholder: this env covers ~226
}
...
def compile_stock_query(
    ...
    *,
    ...
    # ── Universe ────────────────────────────────────────────────────
    uni = spec["universe"]
    where_parts.append(_compile_universe(uni, add_param))
...
# ── Internal helpers ────────────────────────────────────────────────
def _compile_universe(uni: Dict[str, Any], add_param) -> str:
    t = uni["type"]
    ref = uni["ref"]
    if t == "index":
        clause = INDEX_REFS.get(ref.upper())
        if not clause:

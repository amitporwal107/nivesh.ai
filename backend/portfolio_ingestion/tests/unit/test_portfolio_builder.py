...
def test_full_payload_maps_every_leaf() -> None:
    payload = ParsedData(
        ...
        mutual_funds=[MfAmc(amc="HDFC", folios=[
            MfFolio(folio="F-001", schemes=[
    ...
    assert classes == ["equity", "equity", "insurance", "mf", "nps"]
...
def test_mf_dedupe_on_amc_folio_scheme() -> None:
    ...
    payload = ParsedData(mutual_funds=[
        MfAmc(amc="HDFC", folios=[MfFolio(folio="F-1", schemes=[s, s])]),
    ])
    rows = _build(payload)
    assert len(rows) == 1
...
def test_mf_different_folio_is_not_a_duplicate() -> None:
    ...
    payload = ParsedData(mutual_funds=[
        MfAmc(amc="HDFC", folios=[
            MfFolio(folio="F-1", schemes=[s1]),
            MfFolio(folio="F-2", schemes=[s2]),
        ]),
    ])
    rows = _build(payload)
    assert len(rows) == 2
...
def test_source_trace_on_every_row_has_required_keys() -> None:
    payload = ParsedData(
        ...
        mutual_funds=[MfAmc(amc="A", folios=[MfFolio(folio="F", schemes=[
